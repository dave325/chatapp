...
export class AppComponent implements OnDestroy, OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    let user: any;
    if (userExist) {
      user = await this.http.get("http://localhost:3001/users/?user=" + existingUser).toPromise();

    } else {
      user = await this.http.get("http://localhost:3001/users/").toPromise();
    }
    console.log(user)
    window.sessionStorage.setItem('X-DAVE-TEST', user.currentUser.toString())

    this.totalUsers = userExist ? existingUser : user.currentUser;
